class Codex {
    ...
    public update() {
        ...
        for (let g = 0; g < this.constants.GEOMETRIES_COUNT; g++) {
            ...
            for (i = 0; i < positionArray.length; i += 3) {
                if (i >= this.vcap) {
                    let s0 = Math.floor((i - this.vcap) * 0.655);
                    s = timeDomain[g ? timeDomain.length - s0 : s0];
    ...
    public resize(width: number, height: number) {
        ...
        for (let i = 0; i < this.constants.GEOMETRIES_COUNT; i++) {
            ...
            const geometry = new this.THREE.PlaneBufferGeometry(geoWidth, 7500, this.gridWidth - 1, this.gridDepth - 1);
            ...
            const j = i ? -1 : 1;
            mesh.position.x = j * (geoWidth/2 - 100);
