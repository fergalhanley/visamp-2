...
class Codex {
    ...
    public update() {
        ...
        const level = (sum / 512) / 256 || 1;

        const theta = t % Math.PI * 2;
        const ocil = Math.sin(t) * 10;
        const max = Math.abs(this.prop.get("custom_prop_points") * ocil);
        ...
        for (let i = 0; i < max; i++) {
            ...
            this.ctx.strokeStyle = this.colorGen(this.width, this.height, x1, y1, i, level);
            ...
            x2 = this.cx + (this.r * zoom * 4 * Math.cos(this.deg * i));
            y2 = this.cy + (this.r * zoom * 4 * Math.sin(this.deg * i));
