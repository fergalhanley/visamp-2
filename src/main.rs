...
impl Value {
    ...
    pub fn into_f32(self) -> f32 {
        match self {
            Value::Float(fVal) => fVal as f32,
            Value::Integer(iVal) => iVal as f32,
            other => panic!("Expected Color, got {:?}", other),
        }
    }
}
...
fn main() {
    ...
    let parse_result = VisampDSLParser::parse(Rule::script, &dsl_source)
...
// Create a static variable to hold your Model. Make sure Model implements Clone.
static MODEL: OnceCell<Model> = OnceCell::new();

// This is the free function pointer that the nannou API expects.
fn model_fn(app: &App) -> Model {
    // Create a new window or perform other initialization
    app.new_window().view(view).build().unwrap();
    // Retrieve the model from the global state.
    // We clone it here since MODEL.get() returns a shared reference.
    MODEL.get().expect("Model not initialized").clone()
}

// Instead of get_model_fn returning a closure, we set up the global variable.
fn setup_model(script: Script) {
    let model = Model::from_script(&script);
    // Initialize the global MODEL. If already set, you may choose how to handle it.
    MODEL.set(model).unwrap();
}

// A simple interpreter that, for now, just prints out the AST.
fn interpret(script: Script) {
    println!("Interpreting DSL:");
    // Set up the global model first
    setup_model(script);
    // Now pass the free function pointer to nannou::app
    nannou::app(model_fn)
        .update(update)
        .run();
}
    ...
    match function_call.namespace.as_str() {
        "draw" => {
            match function_call.function.as_str() {
                "background" => {
                    let mut drawBackground = draw.background();
                    for arg in function_call.args.iter() {
                        match arg.name.as_str() {
                            "color" => {
                                drawBackground = drawBackground.color(BLACK);
                            },
                            _ => {},
                        }
                    }
                ...
                "polygon" => {
                    if let Some(points) = function_call.args.iter().find(|arg| arg.name == "points") {
                        ...
                        let mut drawPolygon = draw.polygon().points(evaluated_points);
                        for arg in function_call.args.iter() {
                            match arg.name.as_str() {
                                "color" => {
                                    let evaluated = evaluate_expression(&arg.expression, model, decels);
                                    drawPolygon = drawPolygon.color(evaluated.into_srgba_f32());
                                },
                                "rotate" => {
                                    let evaluated = evaluate_expression(&arg.expression, model, decels);
                                    drawPolygon = drawPolygon.rotate(evaluated.into_f32());
                                },
