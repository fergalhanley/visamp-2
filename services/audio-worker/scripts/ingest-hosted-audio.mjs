...
function verifyOutputLoudness(measurement, label) {
  ...
  if (!Number.isFinite(truePeak) || truePeak > RENDITION_PEAK_CEILING_DBTP) {
    throw new Error(
      `${label} rendition peaked at ${truePeak} dBTP, above the ` +
        `${RENDITION_PEAK_CEILING_DBTP} ceiling`,
    );
  }
}
...
/** Legacy queued lossless uploads may use pending agreements, but never
 * terminated ones. New MP3 uploads bypass this worker entirely.
 * Mirrors uploadLicenceGrantsIngest in lib/hosted-audio/upload-rules.ts.
 */
