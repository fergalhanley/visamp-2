...
beforeEach(() => {
  ...
  removeWidget = vi.fn();
  ready = vi.fn(() => { throw new Error("Remove async/defer before using turnstile.ready()"); });
  window.turnstile = Object.assign({ render: renderWidget, remove: removeWidget }, { ready });
  auth.signUp.mockResolvedValue({ data: { session: null }, error: null });
  auth.resetPasswordForEmail.mockResolvedValue({ data: {}, error: null });
});
afterEach(() => { cleanup(); delete window.turnstile; vi.unstubAllEnvs(); vi.clearAllMocks(); });

describe("Turnstile async lifecycle", () => {
  it("renders after script load, cleans up, and remounts without calling ready", () => {
    const onToken = vi.fn();
    const view = render(<Turnstile key="first" siteKey="test" onToken={onToken} />);
  ...
  it("keeps a live widget after Strict Mode replays effects", () => {
    render(<StrictMode><Turnstile siteKey="test" onToken={vi.fn()} /></StrictMode>);
    expect(renderWidget.mock.calls.length - removeWidget.mock.calls.length).toBe(1);
    expect(ready).not.toHaveBeenCalled();
  });

  it("shows widget errors without crashing or issuing a token", () => {
    renderWidget.mockImplementation(() => { throw new Error("Widget failed"); });
    const onToken = vi.fn();
    render(<Turnstile siteKey="test" onToken={onToken} />);
    expect(screen.getByRole("alert").textContent).toContain("security check couldn’t load");
    expect(onToken).toHaveBeenLastCalledWith(null);
  });
  ...
  it("handles asynchronous widget failures and cleanup failures", () => {
    ...
    act(() => { expect(options["error-callback"]()).toBe(true); });
    expect(screen.getByRole("alert")).toBeTruthy();
    removeWidget.mockImplementation(() => { throw new Error("Already removed"); });
...
it("Create account and Forgot password remount CAPTCHA and call the existing auth flows", async () => {
  ...
  await waitFor(() => expect(auth.signUp).toHaveBeenCalledWith({
    email: "creator@example.com", password: "example-password",
    options: { captchaToken: "signup-token", emailRedirectTo: `${window.location.origin}/auth/confirm?next=%2Faccount` },
  }));
  ...
  const reset = screen.getByRole("button", { name: "Send reset link" }) as HTMLButtonElement;
  expect(reset.disabled).toBe(true);
  act(() => options.callback("reset-token"));
  fireEvent.click(reset);
  await waitFor(() => expect(auth.resetPasswordForEmail).toHaveBeenCalledWith("creator@example.com", {
  ...
  expect(screen.getByRole("heading", { name: "Sign in to VisAmp" })).toBeTruthy();
  expect(ready).not.toHaveBeenCalled();
