...
interface CommentsThreadProps {
  vis: Visualisation;
  /**
   * Whether the thread is on screen. Gates the load, so a thread that is only
   * mounted — behind a closed dialog — costs nothing.
   */
  active: boolean;
  className?: string;
  /**
   * The list grows to fill by default, which is what the gallery page wants.
   * The popup caps it instead, since a dialog has no height of its own to
   * flex against.
   */
 ...
 *
 * The list and the composer, without a container: the popup wraps this in a
 * dialog, the artist gallery drops it straight into the page.
 */
export function CommentsThread({
  ...
  const { user } = useAuth();
  const { comments, error, post, remove } = useComments(vis.id, active);
  ...
  return (
    <div className={cn("flex min-h-0 flex-col gap-3", className)}>
      {comments === null ? (
        ...
        <ul
          className={cn("min-h-0 flex-1 space-y-3 overflow-y-auto pr-1", listClassName)}
        >
