...
export function PerformanceView({
  ...
  return (
    <section className="set-performance">
        ...
        {p.embedded ? (
          <>
            ...
            {onPopout && (
              <button
                type="button"
                onClick={p.popout}
                className="absolute right-3 top-3 z-10 rounded-md border border-white/25 bg-black/70 px-3 py-2 text-xs text-white hover:bg-black/90"
              >
                Pop out output
              </button>
            )}
          ...
          (poppedOut ?? (
            <div className="set-output-status">
              ...
              <br />
              <small>
                Rendering and audio are running in the output window.
              </small>
            </div>
          ))
        )}
      ...
      {p.errors.length > 0 && (
        <div role="alert">
          {onPopout && <button onClick={p.restart}>Restart Output</button>}
          {p.errors.map((e, i) => (
            <p key={i}>{e}</p>
          ))}
        </div>
