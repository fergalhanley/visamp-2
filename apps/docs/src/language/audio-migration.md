...
- Audio arrays now contain floats. Array indices, integer arguments and range
  bounds/steps implicitly floor them. Use `value \ 1` if truncation toward zero
  is intended; bitwise operands still need an explicit integer conversion.
