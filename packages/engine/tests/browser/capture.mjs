...
export async function verifyCapture(engine, step, load) {
  ...
  const capture = async () => {
    ...
    assert(bitmap.width === 1280 && bitmap.height === 720, 'fixed capture dimensions');
    const canvas = document.createElement('canvas');
    canvas.width = bitmap.width; canvas.height = bitmap.height;
    const ctx = canvas.getContext('2d');
    ctx.drawImage(bitmap, 0, 0); bitmap.close();
    return {canvas, pixel: (x = 640, y = 360) => [...ctx.getImageData(x, y, 1, 1).data]};
  };
  load(`context 2d
render {
...
render {
 ...
 draw::cube(size: 2.0, color: color::rgb(r: 0.25))
  ...
  assert(contexts.size === 1, `expected one capture WebGL context, got ${contexts.size}`);
  assert(engine.get_properties() === before, 'capture changed live script properties');
  assert(document.querySelector('#visamp-stage canvas') === liveCanvas, 'live canvas replaced');
  assert(liveCanvas.width === 640 && liveCanvas.height === 480, 'live dimensions changed');
  step();
  assert(engine.get_properties() !== before, 'animation did not resume');
  const gl = liveCanvas.getContext('webgl2');
  assert(!gl.isContextLost() && gl.getError() === gl.NO_ERROR, 'live WebGL damaged');
  const livePixel = new Uint8Array(4);
  gl.readPixels(320, 240, 1, 1, gl.RGBA, gl.UNSIGNED_BYTE, livePixel);
  near([...livePixel], [64,0,0,255], 'live mesh still renders');
  checks.push('3D meshes, filters, opaque background; 20 captures reuse one context; live state/playback preserved');

  engine.set_asset_points('capture-model', new Float32Array([-1,0,0, 0,0,0, 1,0,0]));
  engine.set_asset_texture('capture-sprite', 1, 1, new Uint8Array([0,255,0,128]));
  load(`context 3d
render {
 camera::orthographic(height: 4.0)
...
render {
 for i in 0..1.5 { draw::cube() }
  ...
  let failure;
  try { await engine.capture_frame(); } catch (error) { failure = error; }
  assert(failure instanceof Error, 'capture rejection must be a JavaScript Error');
  assert(failure.message.includes('range end') && failure.message.includes('--> 3:'), 'capture error lacks reason/location');
