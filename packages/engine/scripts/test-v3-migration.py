...
Usage: test-v3-migration.py [postgres-container]
"""
...
spec = importlib.util.spec_from_file_location('prepare', Path(__file__).with_name('prepare-v3-migration.py'))
...
try:
    with tempfile.TemporaryDirectory() as temp:
        rows = [dict(id='00000000-0000-4000-8000-000000000001',source='render { draw::background(color: color::rgb(transparent: 0.25)) }',updated_at='2026-09-16T00:00:00Z'),dict(id='00000000-0000-4000-8000-000000000002',source='render { draw::rect(w: 2, h: 3) }',updated_at='2026-09-16T00:00:00Z')]
        output=Path(temp)/'migration'; module.prepare(rows,output)
        ...
        assert sql("select count(*) from public.visualisations where source like '%transparent:%' or source like '%(w:%';")=='0'
        ...
        assert 'a: 1.0' in sql("select source from public.visualisations order by id limit 1;")
        print('PASS: forward, rerun, rollback, metadata preservation, revision conflict, atomic abort and rollback conflict')
finally:
