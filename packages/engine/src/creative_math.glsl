...
float creative_random(float seed, float index) { seed=floor(seed); index=floor(index); if (!creative_index(seed) || !creative_index(index)) return creative_nan(); return float(creative_hash(uint(index) ^ creative_hash(uint(seed))) >> 8) / 16777216.0; }
float creative_noise(float x,float y,float z,float seed) {
 seed=floor(seed); vec3 p=vec3(x,y,z); if (!creative_index(seed) || any(isnan(p)) || any(isinf(p)) || any(greaterThan(abs(p),vec3(1000000.0)))) return creative_nan();
